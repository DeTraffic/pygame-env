...
class IntersectionGame(pygame.sprite.Sprite):
    ...
    def __car_control(
        self, car_group, to_remove, rect_x, rect_y, car_direction, reward
    ):
        for car in car_group:
            ...
            if (
                x > self.width
                or x < 0
                or x < -(self.lane_width / 2)
                or y > self.height
                or y < -(self.lane_width / 2)
            ):
                reward += car.reward
            ...
            look_ahead_rect = pygame.Rect(
                rect_x(x, car.speed), rect_y(y, car.speed), 1, 1
            )
    ...
    def update(self, traffic_light_action):
        ...
        if (now - self.last_time) >= 100:
            ...
            # Left
            if traffic_light_action[0]:
                self.state["left_open"] += 1
                self.state["left_close"] = max(
                    0,
                    self.state["left_close"]
                    - (
                        max(
                            0,
                            (self.state["left_close"] - (self.state["left_close"] - 1))
                            * (
                                self.state["left_close"]
                                + (self.state["left_close"] - 1)
                            ),
                        )
                    ),
                )
            else:
                self.state["left_open"] = 0
                self.state["left_close"] += 1

            # Right
            if traffic_light_action[1]:
                self.state["right_open"] += 1
                self.state["right_close"] = max(
                    0,
                    self.state["right_close"]
                    - (
                        max(
                            0,
                            (
                                self.state["right_close"]
                                - (self.state["right_close"] - 1)
                            )
                            * (
                                self.state["right_close"]
                                + (self.state["right_close"] - 1)
                ...
                self.state["right_open"] = 0
                self.state["right_close"] += 1

            # Top
            if traffic_light_action[2]:
                self.state["top_open"] += 1
                self.state["top_close"] = max(
                    0,
                    self.state["top_close"]
                    - (
                        max(
                            0,
                            (self.state["top_close"] - (self.state["top_close"] - 1))
                            * (self.state["top_close"] + (self.state["top_close"] - 1)),
                        )
                    ),
                )
            else:
                self.state["top_open"] = 0
                self.state["top_close"] += 1

            # Bottom
            if traffic_light_action[3]:
                self.state["bottom_open"] += 1
                self.state["bottom_close"] = max(
                    0,
                    self.state["bottom_close"]
                    - (
                        max(
                            0,
                            (
                                self.state["bottom_close"]
                                - (self.state["bottom_close"] - 1)
                            )
                            * (
                                self.state["bottom_close"]
                                + (self.state["bottom_close"] - 1)
        ...
        center_rect = pygame.Rect(
            self.lane_height, self.lane_height, self.center_width, self.center_height
        )

        self.state["left_center"] = len(
            center_rect.collidelistall(self.left_cars_group.sprites())
        )
        self.state["right_center"] = len(
            center_rect.collidelistall(self.right_cars_group.sprites())
        )
        self.state["top_center"] = len(
            center_rect.collidelistall(self.top_cars_group.sprites())
        )
        self.state["bottom_center"] = len(
            center_rect.collidelistall(self.bottom_cars_group.sprites())
        )
        ...
        self.state["left_car_count"] = len(self.left_cars_group.sprites())
        self.state["right_car_count"] = len(self.right_cars_group.sprites())
        self.state["top_car_count"] = len(self.top_cars_group.sprites())
        self.state["bottom_car_count"] = len(self.bottom_cars_group.sprites())

        game_over = False

        reward -= self.state["left_waiting"] * (
            max(0, self.state["left_close"] / 60) ** (4 / 3)
        )
        reward -= self.state["right_waiting"] * (
            max(0, self.state["right_close"] / 60) ** (4 / 3)
        )
        reward -= self.state["top_waiting"] * (
            max(0, self.state["top_close"] / 60) ** (4 / 3)
        )
        reward -= self.state["bottom_waiting"] * (
            max(0, self.state["bottom_close"] / 60) ** (4 / 3)
        )
